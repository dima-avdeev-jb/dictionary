...
val dictionary1
    get() = Dictionary(
     ...
     words = buildList {
      ...
      word("fill", "заполнять")
      word("assert", "утверждать")
      word("overlap", "перекрытие")
      ...
      word("revolute (revolution)", "вращательное (переворот, оборот)")
      word("5 revolutions per minute (5 rpm)", "5 оборотов в минуту")
      ...
      word("flow", "течение, течь ")
      word("flow of control", "поток контроля (управления)")
      word("memory leaks", " - утечки памяти")
      word(
       "Доме́н (фр. domaine)",
       "область; единица структуры (владения короля или владение какого-либо феодала в Средние века)"
      )
      word("route", "маршрут")
      word("workaround", "обход (костыль в программировании)")
      word("pick", "выбирать")
      word("fetch", "получать")
      word("pull", "тянуть")
      ...
      word("spire", "шпиль")
      word("boilerplate", "шаблонный, повторяющийся, однотипный")
      word("stage", "сцена, стадия.")
      ...
      word("escaped characters", "экранированные символы \\")
      ...
      word("suggest", "предлагать")
      word("hood", "капот (under the hood - под капотом)")
      ...
      word("pivot", "ось поворота, стержень")
      word("aspect ratio", " соотношение сторон")
      ...
      word("intermediate", " representation - промежуточное представление")
      word("milestone", "шаг, веха истории (mile stone - мильный камень, или отметка на дороге)")
      word("confine", "ограничивать")
      word("conflate", "объединять")
      word("propagation", "распространение")
      word("suppress", "подавлять")
      word("fine", "хорошо (мелко)")
      word("coarse", "грубо")
      ...
      word("provisioning profile", "Профиль обеспечения, provision - профиль")
      ...
      word("acquire", "приобретать")
      word("no issues identified", "проблем не обнаружено")
      word("flagged", "помечен")
      word(
      ...
      word("autonomously", "автономно")
      word("service terms", "условия обслуживания")
      word("eligible for", "право на (отвечает требованиям)")
      word("content rating", "рейтинг контента (возврастные ограничения)")
      ...
      word("stick", "палка, придерживаться")
      ...
      word("pane", "панель")
      word("stump", "пень")
      word("requires opt in", "требует согласия")
      ...
      word("criss-cross way", "перекрестный путь")
      word("springboard", "трамплин")
      word("bearer", "предъявитель")
      word("circuit breaker", "выключатель")
      word("perhaps", "возможно, может быть")
      word("topic", "тема")
      word("indent", "отступ")
      word("bidirectional", "двунаправленный")
